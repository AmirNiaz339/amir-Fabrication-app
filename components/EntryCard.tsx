...
const EntryCard: React.FC<EntryCardProps> = ({ entry, isAdmin, theme, onDelete, onPreview }) => {
  ...
  const cardBg = "bg-white border-slate-200 text-slate-900 shadow-lg";
  
  // Format numbers to remove decimals
  const roundVal = (val: string | undefined) => {
    if (!val) return '0';
    const parsed = parseFloat(val);
    return isNaN(parsed) ? val : Math.round(parsed).toString();
  };
  ...
  const Row = ({ label, value, isFull = true, isYellow = true }: { label: string, value: any, isFull?: boolean, isYellow?: boolean }) => (
    <div className={`flex border-b border-slate-300 last:border-b-0 ${isFull ? 'w-full' : 'w-1/2'}`}>
      <div className="bg-black text-white px-2 py-1.5 text-[9px] font-black uppercase shrink-0 w-[110px] flex items-center border-r border-white/10">
        {label}
      </div>
      <div className={`px-2 py-1.5 text-[10px] font-bold flex-1 overflow-hidden break-words min-h-[30px] flex items-center leading-tight ${isYellow ? 'bg-[#fff9e6]' : 'bg-white'}`}>
        {value || '-'}
      </div>
    </div>
  );
    ...
    <div className={`rounded-3xl overflow-hidden border transition-all duration-300 group flex flex-col ${cardBg}`}>
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="border border-slate-400 rounded overflow-hidden shadow-sm bg-slate-100">
          <div className="bg-[#337ab7] text-white py-1.5 text-center text-[10px] font-black uppercase tracking-widest">Product Detail</div>
          ...
          <Row label="Hir3" value={entry.lookupData?.hir3} />
          <Row label="Hir5" value={entry.lookupData?.hir5} />
        ...
        <div className="flex gap-2">
          <button onClick={() => onPreview(entry.images[0].url)} className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-all">
            <Maximize2 className="w-3.5 h-3.5 text-blue-600"/> Full Screen
          </button>
          {isAdmin && (
            <button onClick={() => onDelete(entry.id)} className="p-2.5 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-xl border border-red-100 transition-all">
        ...
        {/* Tiny Footer */}
        <div className="pt-3 border-t border-slate-100 flex justify-between items-center text-[8px] font-black uppercase opacity-40">
